...
def retrieve_positions_from_dataframe(latitude, longitude):
    ...
    for x, y in enumerate(latitude):
        if (
            not math.isnan(latitude[x])
            and not math.isnan(longitude[x])
            and not latitude[x] is None
            and not longitude[x] is None
        ):
...
def extract_data_from_fit(file_name):
    ...
    if os.path.isfile(f"{file_name}.pickle"):
        ...
        df1 = pd.DataFrame({"pace": pace, "hr": hr, "power": power})
        ...
        positions = retrieve_positions_from_dataframe(
            df.latitude.to_list(), df.longitude.to_list()
        )
        ...
        for x, y in enumerate(distance):
            if y is not None and not latitude[x] is None and not longitude[x] is None:
                if math.floor(distance[x] / 1000) > last_kilometer:
                    ...
                    if len(positions_for_km) < last_kilometer + 1:
                        ...
                        positions_for_km[last_kilometer].append(
        ...
        for t, x in enumerate(df.to_dict("records")):
            if (
                not math.isnan(x["distance"])
                and math.floor(x["distance"] / 1000) > kilometers_count
            ):
                kilometers_count += 1
                ...
                for ll in range(0, len(altitude_segment) - 1):
                    delta = altitude_segment[ll + 1] - altitude_segment[ll]
                    if delta > 0:
                        tot_ascent += delta
                        ascent += delta
                    else:
